...
#
adaptiveDFBA.I <- function (models,
			substrateRxns,
                        initConcentrations,
                        initBiomass,
                        timeStep,
    	...
    	for (i in 1:length(models)) {
        	if(is(model, "modelorg")) { 
        		cat('The model number ', i, ' is  ', models[i]@mod_name, '\n')
                	...
                	cat(models[i]@mod_name, " is not an object of the class 'modelorg'\n")
                }
        }
    }
    ...
    for (m in models) {
	    ...
	    biomassIdx <- which(m@react_id %in% biomassRxn)
        ...
        if (biomassRxn == '' || missing(biomassRxn)) {
            ...
            #	right
            #
            biomassIdx <- grep('biomass(m)', m@react_id, ignore.case=TRUE)[1]
            # if found use it
            if (length(biomassIdx) != 0) {
                biomassRxn <- m@react_id[biomassIdx]
	        ...
	        biomassIdx <- which(sybil::obj_coef(m) != 0)[1]
	        biomassRxn <- sybil::m@react_id[biomassIdx][1]
	    }
        ...
        excReactInd <- (react_id(m) %in% react_id(excReact));
        # Exclude reactions with concentrations that will not be changed 
        exclUptakeRxnsInd <- is.element(m@react_id, exclUptakeRxns);
        # update exchange reaction index
        excReactInd <- excReactInd & !exclUptakeRxnsInd;
        # get reaction names
        excRxnNames <- m@react_id[excReactInd];

        ##--------------------------------------------------------------------------##
        # Find substrate reactions
     
        substrateRxnsInd=(m@react_id %in% substrateRxns)
        if (verboseMode > 1) {
        ...
        concentrations=rep(0,length(m@react_id))
        #
        ...
        names(concentrations) <- m@react_id
        #
        # XXX j XXX
        ...
        for (i in 1:length(substrateRxns)) {
            ...
            # in the same order!!!
            concentrations[m@react_id == substrateRxns[i]] <- initConcentrations[i]
    	}
	    ...
	    #for (i in substrateRxns) {
            for (i in 1:length(concentrations)) {
              if (concentrations[i] != 0) {
                cat(react_id(m)[i], concentrations[i], '\n', sep="	")
              }
            }
            cat('\n')
        }
	    ...
	    for (i in 1:length(excReactInd)) {
                if (excReactInd[i]) 
                    cat(paste(i, react_id(m)[i], concentrations[i], sep="	" ), '\n')
            }
	    cat('\n')
        }
   	...
   	##irene: esto creo que deberia hacerse creando listas en cada uno de los argumentos
   	##que le damos a la funcion para que cada modelo tenga su biomasa
    	biomass = initBiomass;
    	if (verboseMode > 0) {
        ...
        concentrationMatrix <- cbind(concentrations[excReactInd]);
        biomassVec <- biomass;
        timeVec <- 0;
        ...
        all_fluxes <- cbind(rep(0, length(m@react_id)));	# start with flux=0 at t=0
	         ...
	         if (verboseMode > 3) {
	             ...
	             if (verboseMode > 3) {
		             ...
		             # get old limits for verboseMode reporting below
                             oldlow <- lowbnd(m)[m@react_id == rxnName]
		             oldupp <- uppbnd(m)[m@react_id == rxnName]

		             # update corresponding limits according to original reaction name
                             if (grepl("\\[low\\]", dcr)) {
                                 # reaction.low.
                                 lowbnd(m)[m@react_id == rxnName] <- rxnValue
			         ...
			         # note: this will be done afterwards all at once
                                 ...
                                 # reaction.upp.
                                 uppbnd(m)[m@react_id == rxnName] <- rxnValue
                             } else {
                                 # reaction
                                 lowbnd(m)[m@react_id == rxnName] <- rxnValue
                                 # a negative lower bound implies uptake, so we change the sign
                         ...
                         if (verboseMode > 3) {
                             cat(info,
                                 ...
                                 ' oup ', oldupp,
                    	         ' exp ', rxnValue,
                                 ' nlo ', lowbnd(m)[m@react_id == rxnName],
                                 ' nup ', uppbnd(m)[m@react_id == rxnName],
...
##        originalUptake[excReactInd] <- -lowbnd(model)[excReactInd]
	    ...
	    if (verboseMode > 0) {
		    ...
		    # modify its concentration as requested
                    if (stepNo <= dim(nutrientChanges[nutrient])[1]) {
                        ...
                        for (m in models) {
		            concentrations[m@react_id == nutrient] <- 
                                concentrations[m@react_id == nutrient] +
	...
	# or alternatively for all (we'll only use exchange reactions anyway)
        ...
        for (m in models) {
            uptakeBound =  concentrations/(biomass*timeStep);
            cat('The uptake bound for the model ', m@mod_name, 'in the step = ', stepNo, ' is = ', uptakeBound)
	    ...
	    lpmod <- sybil::sysBiolAlg(m, algorithm = "fba", ...)
            ...
            if (verboseMode > 3) {
                lplow <- getColsLowBnds(problem(lpmod),
                                        which(c(rep(TRUE, length(m@react_id)))))
	    ...
	    if (method == "directFBA") {
		    ...
		    for (i in 1:length(sol$fluxes)) {
    		        if (sol$fluxes[i] != fba.sol$fluxes[i]){
                            if (verboseMode > 3) 
        		        cat(err, 'DIFF', i, m@react_id[i], sol$fluxes[i], fba.sol$fluxes[i], '\n', sep='	')
                            n = n+1
     		        }
	            }
                    if (n > 0)
		    ...
		    # then solve MTF with the computed objective
                ...
                # NOTE that we could also call MTF with retOptSol=FALSE
	    ...
	    ### j ###
	        ...
	        tabFVAvalues <- paste(m@mod_name, 'ADFBA-FVA-', stepNo, '.tab', sep='')
                ...
                for (i in 1:nreact) {
                    if (verbose > 6) {
                        print(paste('FVA,', stepNo, ',', i, ',', m@react_id[i], ',', 
    			            lp_obj(ranges)[i], ',', lp_obj(ranges)[i+nreact]))
                    }
                    out <- paste(i, '	', m@react_id[i], '	', 
    		                lp_obj(ranges)[i], '	', lp_obj(ranges)[i+nreact])
	    ...
	    mu_bmidx <- sol$fluxes[biomassIdx];
            mu_bmrxn <- sol$fluxes[m@react_id == biomassRxn]
            mu <- mu_bmidx
            
            if (verboseMode > 2) {
	        cat(info, 'SOL$obj:', sol$obj, '\n')
                cat(info, 'mu(obj)', mu_obj, 
            	    'mu(biomassIdx/',biomassIdx,')', mu_bmidx, 
                    'mu(biomassRxn/',biomassRxn,')', mu_bmrxn, '\n')
                cat(info, 'Using MU =', mu, '\n')
                cat(info, 'Max. Biomass rate', uppbnd(m)[biomassIdx], '\n')
                if (mu > (uppbnd(m)[biomassIdx]))
                    cat(err, 'MU', mu, '> biomass[upp]', uppbnd[biomassIdx], '\n')
            }
	    ...
	    # get uptake fluxes after FBA/MTF
            ...
            concentrations[excReactInd]= sum(concentrations[excReactInd]) - sum(uptakeFlux/mu*biomass*(1-exp(mu*timeStep)));
            ...
            if (verboseMode > 2) {
                ...
                for (i in substrateRxns)
                    cat(i, concentrations[m@react_id == i], '\n', sep=" ")
	    ...
	    if (verboseMode > 1) cat('Objective at step', stepNo, '(', stepNo*timeStep ,'h ):', sol$obj, '\n');
            if (verboseMode > 1) cat('Biomass of', m@mod_name, 'at t =', stepNo, '(', stepNo*timeStep, "h ):", biomass, '\n');
    ...
    for (m in models) {
        ...
        row.names(all_fluxes) <- react_id(m);
        colnames(concentrationMatrix) <- timeVec;
        colnames(all_fluxes) <- timeVec[-1];
        print(paste("JR:nSteps", stepNo))
        print("JR: ALL FLUXES")
        print(dim(all_fluxes))
        print("JR: CONC MAT")
        print(dim(concentrationMatrix))
